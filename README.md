...
We plot $\log_2(\text{Probability of success})$ for $n=d=3 ... 9$, in order to verify that the chance of failure is $O(2^{-n})$ (although the probability of failure is not monotone decreasing for all $n$). We also include a histogram of outcomes simulated by AerSimulator.